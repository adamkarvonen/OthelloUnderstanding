        ...
        for layer in layers:
            ...
            if ablation_method == "mean":
                ...
                if ablate_not_selected:
                    ...
                    encoded_BLF[:, :, not_feature_idxs_F] = mean_activations[layer][not_feature_idxs_F]
            else:
...
# combinations = [
#     ("mlp_neuron", "mean", False, False),
#     ("sae_mlp_out_feature", "mean", False, False),
#     ("transcoder", "mean", False, False),
...
#     with open(f"decision_trees/results_{input_location}_{custom_functions[0]}.pkl", "wb") as f:
#         pickle.dump(results, f)

#     with open(
#         f"decision_trees/ablation_results_{input_location}_{ablation_method}_ablate_not_selected_{ablate_not_selected}_add_error_{add_error}_{ablation_custom_function}.pkl",
#         "wb",
#     ) as f:
...
for filename in os.listdir("decision_trees"):
    if filename.startswith("results_") and "games_batch_to_input_tokens_classifier_input_BLC" in filename:
    ...
    if filename.startswith("ablation_results_") and "games_batch_to_input_tokens_classifier_input_BLC" in filename:
        with open(f"decision_trees/{filename}", "rb") as f:
